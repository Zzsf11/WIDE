...
class Resize(object):
    ...
    def __call__(self, image, target):
        ...
        if isinstance(target, list):  # Assuming target is a list of coco annotations
            for idx, ann in enumerate(target):
                if 'bbox' in ann:
                    bbox = ann['bbox']
                    ann['bbox'] = [
                        bbox[0] * image[0].width/original_size[0] ,
                        bbox[1] * image[0].height/original_size[1],
                        bbox[2] * image[0].width/original_size[0],
                        bbox[3] * image[0].height/original_size[1]
                    ]
        else:
            ...
            if isinstance(target, list):  # Assuming target is a list of coco annotations
                for idx, ann in enumerate(target):
                    if 'bbox' in ann:
                        bbox = ann['bbox']
                        ann['bbox'] = [
                            max(0, image[0].width - (bbox[0] + bbox[2])),
                            bbox[1],
                            bbox[2],
                            bbox[3]
                        ]
